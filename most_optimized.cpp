...
void path_checker(
    ...
    for(int node = 0; node < delivery_points - 1; node++){
        ...
        // Pre-calculate A -> S and S -> B transitions for all stations
        vector<double> time_to_station(charging_stations);
        vector<int> loss_to_station(charging_stations);
        vector<double> time_from_station(charging_stations);
        vector<int> loss_from_station(charging_stations);

        for (int s = 0; s < charging_stations; s++) {
            time_to_station[s] = time_for_distance(dis_c[node][s]);
            loss_to_station[s] = charge_loss_for_distance(dis_c[node][s], current_weight);
        ...
        for(int charge = 0; charge <= charge_quantums; charge++){
            ...
            for (int station = 0; station < charging_stations; station++) {
                ...
                double time_to_station_arrival = arrival_time + time_to_station[station];
                int charge_at_station = charge - charge_loss_to_station;

                if(time_to_station_arrival < dp_c[node][station][charge_at_station]){
                    dp_c[node][station][charge_at_station] = time_to_station_arrival;
                }
            }
        }
